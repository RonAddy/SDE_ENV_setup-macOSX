#!/bin/bash
# node (nvm, npm, yarn) purge!
# credit: http://stackoverflow.com/a/11178106/2083544
# repo: https://github.com/brock/node-reinstall/blob/master/node-reinstall

# TODO: Add checks to:
# Delete nvm, yarn, node if installed by brew
  # - check if brew has it in Cellar
  # - delete the symlink
  # - (use brew commands)
#Add option to install using brew
...
usage () {
  printf "%s\n" "node-purge"
  printf "\t%s\n" "completely purges Node, NVM, Yarn, NPM and any global node modules."
  printf "\t%s\n" "It purges Node using NVM or Nave"
  printf "\n"
  printf "%s\t%s\n" "Usage:" "node-purge [--nave|--nvm|--nvm-latest] [-h|--help] [-v|--version] [NODE_VERSION]"
  printf "\n"
  printf "%s\n" "Commands:"
  printf "\n"
  printf "\t%s\t\t\t%s\n" "node-purge" "purge node and npm using nvm, purge yarn, then purge nvm"
  printf "\t%s %s\t%s\n" "node-purge" "[-h|--help]" "show help"
  printf "\t%s %s\t%s\n" "node-purge" "[-v|--version]" "show the node-purge version number"
  # printf "\t%s %s\t%s\n" "node-purge" "[-f|--force]" "installs defaults without user confirmation"
  # printf "\t%s %s\t\t%s\n" "node-purge" "--nave" "re-install using nave"
  # printf "\t%s %s\t\t%s\n" "node-purge" "--nvm" "re-install using stable nvm - the default"
  # printf "\t%s %s\t%s\n" "node-purge" "--nvm-latest" "re-install using latest nvm - creationix/nvm:master"
  # printf "\t%s %s\t\t%s\n" "node-purge" "0.12" "specify a default node version - currently ${NODE_VERSION}"
  printf "\n"
}
...
confirm ()
{
  if [[ $FORCE == 0 ]]; then
    echo ""
    echo "Would you like to continue running node-purge?"
    echo ""
...
if [[ -z $IS_NPM_MISSING ]]; then
  ...
  if [[ -n $GLOBAL_MODULES ]]; then
    echo "Current global npm modules:"
    echo $GLOBAL_MODULES
  else
    # echo "===== ALERT! ====="
