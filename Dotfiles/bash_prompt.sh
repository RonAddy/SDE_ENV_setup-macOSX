...
prompt_git() {
    ...
    if $GIT_DIFF_IN_PROMPT; then
      ...
      print_statement="${WHITE} 👉🏾 ${style_branch}${git_info}${RESET}" 

      # Combine the branch name and state information
      if [[ $git_state ]]; then
          git_info="$git_info${RESET} ⚠️ ($git_state${RESET})"
          print_statement="${WHITE} 👉🏾 ${style_branch_changes}${git_info}${RESET}" 
      fi
    fi
